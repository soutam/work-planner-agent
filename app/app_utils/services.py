...
def get_memory_service():
    """Process-wide memory service: Vertex AI Memory Bank when configured, else in-memory."""
    ...
    if agent_engine_id := (
        os.environ.get("GOOGLE_CLOUD_AGENT_ENGINE_ID") or "4472542821932335104"
    ):
        from google.adk.memory import VertexAiMemoryBankService

        return VertexAiMemoryBankService(
            project=os.environ.get("GOOGLE_CLOUD_PROJECT", "qwiklabs-gcp-01-0482b2deb94b"),
            location=os.environ.get("GOOGLE_CLOUD_AGENT_ENGINE_LOCATION")
            or os.environ.get("GOOGLE_CLOUD_LOCATION")
            or "us-east1",
            agent_engine_id=agent_engine_id,
